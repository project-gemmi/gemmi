...
from chemcomp_planar_fragments import (
    bond_graph,
    cycle_fragments,
    dedup_fragments,
    plane_fragments,
)
from chemcomp_planar_embed import (
    Vec2,
    angle_between,
    build_restraint_maps,
    embed_core,
    get_angle,
    optimize_layout,
    refine_angles,
...
def choose_fragments(cc, core_atoms):
    ...
    frags = dedup_fragments(
        plane_fragments(
            cc,
            core_atoms)
        + cycle_fragments(graph))
    bad_planes = []
    # Prefer cycles and small articulation planes. Drop broad plane supersets
    # that are just ring+few extras.
    out = []
...
def one_overlap_transform(
    ...
    if x is None or shared not in global_pos:
        return None
    refs = [nb for nb in core_adj[shared]
            if nb in global_pos and nb not in set(frag['atoms'])]
    if not refs:
    ...
    while added:
        ...
        for atom in core_atoms:
            ...
            if len(nbs) != 1 or len(placed_nbs) != 1:
                continue
            center = placed_nbs[0]
            refs = [nb for nb in core_adj[center]
            ...
            for atom in plane['atoms']:
                ...
                if len(placed_nbs) >= 2:
                    ...
                    for ref in refs:
                        ...
                        for sign in (1.0, -1.0):
                            cand = positions[center] + \
                                rotate(base, sign * theta) * d
                            score = 0.0
                            for nb in core_adj[atom]:
                                if nb in positions and nb != center:
                                    target = core_bonds.get(
                                        tuple(sorted((atom, nb))))
                                    if target is not None:
                                        score += abs((cand
                                                      - positions[nb]).length()
                                                     - target)
    ...
    while changed:
        ...
        for frag in fragments:
            ...
            if nb_label in placed_frags:
                continue
            overlap = sorted(
                atom for atom in frag['atoms']
                if atom in global_pos and atom in local[nb_label]
            )
            if not overlap:
                continue
...
def assemble_fragments(cc, fragments, full_core_atoms=None, bridge_planes=None):
    ...
    core_atoms = sorted(full_core_atoms if full_core_atoms is not None else {
                        atom for frag in fragments for atom in frag['atoms']})
    ...
    while q:
        ...
        for nb_label, overlap in edges[cur]:
            ...
            local_pos = local[nb_label]
            if any(
                    atom not in global_pos or atom not in local_pos
        ...
        for nb_label, overlap in edges[cur]:
            ...
            q.append(nb_label)
    if bridge_planes:
        global_pos = place_bridge_planes(
            bridge_planes,
            global_pos,
            core_adj,
            core_angles,
            core_bonds)
        global_pos, placed_frags = attach_pending_fragments(
            fragments,
            placed_frags,
            local,
            global_pos,
    ...
    global_pos = place_terminal_leaves(
        core_atoms,
        global_pos,
        core_adj,
        core_angles,
...
def summarize(cc, core_atoms, positions):
    ...
    for bond in cc.rt.bonds:
        ...
        if a in aset and b in aset and a in positions and b in positions:
            target = bond.value if math.isfinite(
                bond.value) else bond.value_nucleus
...
def main():
    ...
    for item in args.inputs:
        ...
        pos, placed_frags, edges, _local = assemble_fragments(
            cc, fragments, core['atoms'], bridge_planes)
        bond_res, angle_res, max_bond_z, missing = summarize(
            cc, core['atoms'], pos)
        status = (
            'PASS'
            if not missing and max_bond_z <= args.bond_z_limit
            else 'FAIL'
        )
        ...
        print(
            '  placed fragment labels: {}'.format(
                ', '.join(
                    sorted(placed_frags))))
        if bridge_planes:
            print(
                '  bridge planes: {}'.format(
                    ', '.join(
                        p['label'] for p in bridge_planes)))
        if missing:
            print('  missing atoms: {}'.format(', '.join(missing)))
        if not args.summary_only:
            for diff, z, a, b, actual, target in bond_res[:5]:
                print(
                    '    bond  {:>8} {:>8}  {:.4f} vs {:.4f}  z={:.2f}'.format(
                        a, b, actual, target, z))
