  ...
  while (size_t len = copy_line_from_stream(line, 95, infile)) {
    if (line[0] == '!') {
      if (starts_with_ptr(line+1, "Generated by ", &rhs)) {
        ret.generated_by = read_word(rhs);
      } else if (starts_with_ptr(line+1, "SPACE_GROUP_NUMBER=", &rhs)) {
        ret.spacegroup_number = simple_atoi(rhs);
        ...
        auto result = fast_from_chars(rhs, line+len, ret.wavelength);
        if (result.ec != std::errc())
          fail("failed to parse wavelength:\n", line);
      } else if (starts_with_ptr(line+1, "OSCILLATION_RANGE=", &rhs)) {
        auto result = fast_from_chars(rhs, line+len, ret.oscillation_range);
        if (result.ec != std::errc())
          fail("failed to parse:\n", line);
      } else if (starts_with_ptr(line+1, "STARTING_ANGLE=", &rhs)) {
        auto result = fast_from_chars(rhs, line+len, ret.starting_angle);
        if (result.ec != std::errc())
          fail("failed to parse:\n", line);
      } else if (starts_with_ptr(line+1, "STARTING_FRAME=", &rhs)) {
        ret.starting_frame = simple_atoi(rhs);
        ...
        if (starts_with_ptr(endptr, "INPUT_FILE=", &rhs)) {
          ret.isets.push_back({id, "scaled " + read_word(rhs), 0.});
          ...
          auto it = std::find_if(ret.isets.begin(), ret.isets.end(),
                                 [&id](const XdsAscii::Iset& i) { return i.id == id; });
          if (it == ret.isets.end())
            ret.isets.push_back({id, "", w});
          else
            it->wavelength = w;
        }
        ...
        if (ret.isets.empty()) {
          for (XdsAscii::Refl& refl : ret.data) {
            if (refl.iset != 0 && refl.iset != 1)
              fail("unexpected ITEM_ISET " + std::to_string(refl.iset));
            refl.iset = 1;
          }
          ret.isets.push_back({1, "from " + ret.generated_by, ret.wavelength});
        }
        return ret;
      }
    } else {
