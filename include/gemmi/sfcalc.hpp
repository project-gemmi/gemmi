//! @file
//! @brief Direct calculation of structure factors.
//!
...
//! @brief Calculate phase factor exp(2 pi i r * s).
//! @param fpos Fractional position
...
//! @brief Direct structure factor calculator.
//! @tparam Table Scattering factor table type
template <typename Table>
class StructureFactorCalculator {
public:
  using coef_type = typename Table::Coef::coef_type;  //!< Scattering coefficient type

  //! @brief Constructor.
  //! @param cell Unit cell
  StructureFactorCalculator(const UnitCell& cell) : cell_(cell) {}

  //! @brief Set (sin(theta)/lambda)^2 and clear scattering factors cache.
  //! @param hkl Miller indices
  ...
  //! @brief Get scattering factor for element.
  //! @param element Element
  //! @param charge Charge
  //! @return Scattering factor (cached)
  ...
  //! @brief Calculate Debye-Waller factor with isotropic ADP.
  //! @param site Small structure site
  //! @return exp(-u_to_b() * stol2 * u_iso)
  double dwf_iso(const SmallStructure::Site& site) const {
    return std::exp(-u_to_b() * stol2_ * site.u_iso);
  }

  //! @brief Calculate Debye-Waller factor with isotropic ADP.
  //! @param atom Macromolecular atom
  //! @return exp(-stol2 * b_iso)
  double dwf_iso(const Atom& atom) const {
    return std::exp(-stol2_ * atom.b_iso);
  }

  //! @brief Calculate Debye-Waller factor with anisotropic ADP.
  //! @param site Small structure site
  //! @param hkl Miller indices as Vec3
  //! @return exp(-2 pi^2 s.U.s)
  //!
  ...
  //! @brief Calculate structure factor contribution from atom with given scattering factor.
  //! @tparam Site Site type (SmallStructure::Site or Atom)
  //! @param fract Fractional position
  //! @param site Site or atom
  //! @param hkl Miller indices
  //! @param sf Scattering factor
  //! @return Structure factor contribution
  ...
  //! @brief Calculate structure factor contribution from atom.
  //! @tparam Site Site type (SmallStructure::Site or Atom)
  //! @param fract Fractional position
  //! @param site Site or atom
  //! @param hkl Miller indices
  ...
  //! @brief Calculate structure factor from macromolecular model.
  //! @param model Model
  //! @param hkl Miller indices
  ...
  //! @brief Calculate Z part of Mott-Bethe formula.
  //! @param model Model
  //! @param hkl Miller indices
  //! @param only_h If true, only include hydrogen atoms
  //! @return Z contribution to Mott-Bethe formula
  //!
  ...
  //! @brief Get Mott-Bethe correction factor.
  //! @return -mott_bethe_const() / 4 / stol2
  double mott_bethe_factor() const {
    return -mott_bethe_const() / 4 / stol2_;
  }

  //! @brief Calculate structure factor from small molecule structure.
  //! @param small_st Small structure
  //! @param hkl Miller indices
  //! @return Structure factor
  //!
...
private:
  const UnitCell& cell_;  //!< Unit cell reference
  coef_type stol2_;  //!< (sin(theta)/lambda)^2
  std::vector<double> scattering_factors_;  //!< Cached scattering factors
public:
  Addends addends;  //!< Addends (usually f' for X-rays)
};
