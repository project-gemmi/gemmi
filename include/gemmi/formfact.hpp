...
struct GaussianCoef {
  ...
  ExpSum<N+1,Real> precalculate_density_iso(Real B, Real fprime=0) const {
    ExpSum<N+1,Real> prec;
    constexpr Real _4pi = 4 * pi();
    for (int i = 0; i < N; ++i) {
      Real t = _4pi / (b[i]+B);
  ...
  ExpAnisoSum<N+1,Real> precalculate_density_aniso_b(const SMat33<Real>& B,
                                                     Real fprime=0) const {
    constexpr Real m4pi2 = -4 * sq(pi());
    ExpAnisoSum<N+1,Real> prec;
    for (int i = 0; i < N; ++i) {
      SMat33<Real> Bb = B.added_kI(b[i]);
  ...
  ExpAnisoSum<N+1,Real> precalculate_density_aniso_u(const SMat33<float>& U,
                                                     Real fprime=0) const {
