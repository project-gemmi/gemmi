...
void add_symmetry(nb::module_& m) {
  ...
  nb::class_<SpaceGroup>(m, "SpaceGroup")
    .def(nb::new_([](int n) {
          ...
          if (n < INT_MIN + int(sizeof(spacegroup_tables::main) / sizeof(SpaceGroup)))
    ...
    .def("__reduce__", [](const SpaceGroup& self) {
        ...
        int pos = int(&self - spacegroup_tables::main);
        return nb::make_tuple(nb::type<SpaceGroup>(), nb::make_tuple(INT_MIN + pos));
    })
