...
nb::class_<GridBase<T>, GridMeta> add_grid_base(nb::module_& m, const char* name) {
  ...
  grid_base
    ...
    .def("__iter__", [](GrBase& self) {
        return nb::make_iterator(nb::type<GrBase>(), "iterator", self);
    }, nb::keep_alive<0, 1>())
    ;
  return grid_base;
}
...
nb::class_<Grid<T>, GridBase<T>> add_grid_common(nb::module_& m, const std::string& name) {
  ...
  grid
    ...
    .def("__init__", [](Gr* grid, nb::ndarray<nb::numpy, T, nb::ndim<3>> arr,
      ...
      if (sg)
        grid->spacegroup = sg;
    }, nb::arg().noconvert(), nb::arg("cell")=nullptr, nb::arg("spacegroup")=nullptr)
  ...
  masked_grid
    ...
    .def("__iter__", [](Masked& self) {
        return nb::make_iterator(nb::type<Masked>(), "iterator", self);
    }, nb::keep_alive<0, 1>())
    ;
    return grid;
}
