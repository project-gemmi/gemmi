...
void add_asudata(nb::module_& m, const std::string& prefix) {
  ...
  asu_data
    .def("__init__", [](AsuData* p, const UnitCell& unit_cell, const SpaceGroup* sg,
                        nb_miller_array hkl, typename array_for<T>::type values) {
      new(p) AsuData;
      auto h = hkl.view();
      auto v = values.view();
    ...
    .def("__iter__", [](AsuData& self) {
        return nb::make_iterator(nb::type<AsuData>(), "iterator", self.v);
    }, nb::keep_alive<0, 1>())
...
void add_asudata_and_recgrid(nb::module_& m,
  ...
  recgrid
    ...
    .def("get_value_by_hkl", [](RecGr &self, nb_miller_array hkl, double unblur,
