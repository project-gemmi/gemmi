...
void add_mtz(nb::module_& m) {
  ...
  nb::bind_vector<std::vector<Mtz::Dataset>>(m, "MtzDatasets");
  nb::bind_vector<std::vector<Mtz::Column>>(m, "MtzColumns");
  nb::bind_vector<std::vector<Mtz::Batch>>(m, "MtzBatches");
  ...
  mtz
    ...
    .def("column_with_label",
         ...
         nb::arg("label"), nb::arg("dataset")=nullptr,
         nb::rv_policy::reference_internal)
  ...
  pyMtzColumn
    ...
    .def("__iter__", [](Mtz::Column& self) {
        return nb::make_iterator(nb::type<Mtz::Column>(), "iterator", self);
    }, nb::keep_alive<0, 1>())
