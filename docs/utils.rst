...
It is often faster to run a ready-to-use program than to write your own.

gemmi-validate
==============

A CIF validator that checks the syntax and, optionally, also ontology
...
Syntax-level conversion between **CIF and JSON**.
The JSON representation of the CIF data
...
Macromolecular-model-level conversion between **mmCIF, PDB and mmJSON**.
...
**NCS expansion** with the option ``--expand-ncs``:
expands strict NCS, defined in the ``MTRIX`` record (PDB)
