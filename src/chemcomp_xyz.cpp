...
std::vector<std::vector<size_t>> collect_small_plane_groups(
    const ChemComp& cc, const std::map<std::string, size_t>& atom_index) {
  ...
  for (const Restraints::Plane& plane : cc.rt.planes) {
    ...
    for (const Restraints::AtomId& atom_id : plane.ids) {
      ...
      if (it == atom_index.end())
        continue;
      if (is_heavy_atom(cc.atoms[it->second]))
        heavy.push_back(it->second);
    }
    std::sort(heavy.begin(), heavy.end());
    heavy.erase(std::unique(heavy.begin(), heavy.end()), heavy.end());
    if (heavy.size() >= 3 && heavy.size() <= 4)
      groups.push_back(heavy);
  }
...
std::vector<std::vector<size_t>> collect_large_plane_groups(
    const ChemComp& cc, const std::map<std::string, size_t>& atom_index) {
  std::vector<std::vector<size_t>> groups;
  for (const Restraints::Plane& plane : cc.rt.planes) {
    std::vector<size_t> heavy;
...
bool seed_small_plane_group_coordinates(ChemComp& cc,
                                        const std::vector<std::vector<size_t>>& adjacency,
                                        const std::map<std::string, size_t>& atom_index,
  ...
  std::vector<size_t> group_degree(cc.atoms.size(), 0);
  for (size_t idx : group)
  ...
  if (group.size() == 3) {
    ...
    for (size_t idx : group) {
      if (group_degree[idx] == 2)
        center = idx;
      else if (group_degree[idx] == 1)
        leaves.push_back(idx);
    }
    if (center == SIZE_MAX || leaves.size() != 2)
      return false;
    double d1 = cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[0]].id) != cc.rt.bonds.end() ?
                (std::isfinite(cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[0]].id)->value) ?
                 cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[0]].id)->value :
                 cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[0]].id)->value_nucleus) : 1.4;
    double d2 = cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[1]].id) != cc.rt.bonds.end() ?
                (std::isfinite(cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[1]].id)->value) ?
                 cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[1]].id)->value :
                 cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[1]].id)->value_nucleus) : 1.4;
    double theta = rad(default_angle_for_center(adjacency, center));
    if (const Restraints::Angle* angle =
            find_angle_restraint(cc, atom_index, leaves[0], center, leaves[1]))
      if (std::isfinite(angle->value))
        theta = rad(angle->value);
    cc.atoms[center].xyz = Position(x_shift, 0, 0);
    cc.atoms[leaves[0]].xyz = Position(x_shift + d1, 0, 0);
    cc.atoms[leaves[1]].xyz = arbitrary_position_from_angle_local(cc.atoms[leaves[0]].xyz,
                                                                  cc.atoms[center].xyz,
                                                                  d2, theta);
    cc.atoms[leaves[1]].xyz.z = 0.0;
    return true;
  }
  ...
  std::vector<size_t> leaves;
  for (size_t idx : group) {
    if (group_degree[idx] == 3)
      center = idx;
    else if (group_degree[idx] == 1)
      leaves.push_back(idx);
  }
  if (center == SIZE_MAX || leaves.size() != 3)
    return false;
  std::sort(leaves.begin(), leaves.end(),
            [&](size_t a, size_t b) { return cc.atoms[a].id < cc.atoms[b].id; });
  std::vector<double> dist(3, 1.4);
  for (int i = 0; i != 3; ++i) {
    std::vector<Restraints::Bond>::const_iterator bond =
        cc.rt.find_bond(cc.atoms[center].id, cc.atoms[leaves[i]].id);
  ...
  double theta01 = rad(default_angle_for_center(adjacency, center));
  if (const Restraints::Angle* angle =
          find_angle_restraint(cc, atom_index, leaves[0], center, leaves[1]))
    if (std::isfinite(angle->value))
      theta01 = rad(angle->value);
  double theta02 = rad(default_angle_for_center(adjacency, center));
  if (const Restraints::Angle* angle =
          find_angle_restraint(cc, atom_index, leaves[0], center, leaves[2]))
    if (std::isfinite(angle->value))
      theta02 = rad(angle->value);
  double theta12 = rad(default_angle_for_center(adjacency, center));
  if (const Restraints::Angle* angle =
          find_angle_restraint(cc, atom_index, leaves[1], center, leaves[2]))
    if (std::isfinite(angle->value))
      theta12 = rad(angle->value);
  cc.atoms[center].xyz = Position(x_shift, 0, 0);
  cc.atoms[leaves[0]].xyz = Position(x_shift + dist[0], 0, 0);
  cc.atoms[leaves[1]].xyz = Position(x_shift + dist[1] * std::cos(theta01),
                                     dist[1] * std::sin(theta01), 0);
  double phi_a = theta02;
  double phi_b = -theta02;
  ...
  double diff_a = std::fabs(ang_diff(theta01, phi_a) - theta12);
  double diff_b = std::fabs(ang_diff(theta01, phi_b) - theta12);
  double phi = diff_a <= diff_b ? phi_a : phi_b;
  cc.atoms[leaves[2]].xyz = Position(x_shift + dist[2] * std::cos(phi),
                                     dist[2] * std::sin(phi), 0);
  return true;
}

bool seed_large_plane_group_coordinates(ChemComp& cc,
                                        const std::vector<std::vector<size_t>>& adjacency,
                                        const std::map<std::string, size_t>& atom_index,
                                        const std::vector<size_t>& group,
                                        double x_shift) {
  ...
  for (size_t idx : group) {
    int degree = 0;
    for (size_t nb : adjacency[idx])
      if (std::find(group.begin(), group.end(), nb) != group.end() && is_heavy_atom(cc.atoms[nb]))
        ++degree;
  ...
  if (center == SIZE_MAX || best_degree < 2)
    return false;
  ...
  size_t a = neighbors[0];
  size_t b = neighbors[1];
  double da = 1.4;
  double db = 1.4;
  std::vector<Restraints::Bond>::const_iterator bond =
      cc.rt.find_bond(cc.atoms[center].id, cc.atoms[a].id);
  if (bond != cc.rt.bonds.end()) {
    double value = std::isfinite(bond->value) ? bond->value : bond->value_nucleus;
    if (std::isfinite(value))
      da = value;
  }
  bond = cc.rt.find_bond(cc.atoms[center].id, cc.atoms[b].id);
  if (bond != cc.rt.bonds.end()) {
    double value = std::isfinite(bond->value) ? bond->value : bond->value_nucleus;
    if (std::isfinite(value))
      db = value;
  ...
  double theta = rad(default_angle_for_center(adjacency, center));
  if (const Restraints::Angle* angle = find_angle_restraint(cc, atom_index, a, center, b))
    if (std::isfinite(angle->value))
      theta = rad(angle->value);
  cc.atoms[center].xyz = Position(x_shift, 0, 0);
  cc.atoms[a].xyz = Position(x_shift + da, 0, 0);
  cc.atoms[b].xyz = arbitrary_position_from_angle_local(cc.atoms[a].xyz,
                                                        cc.atoms[center].xyz, db, theta);
  cc.atoms[b].xyz.z = 0.0;

  if (neighbors.size() >= 3) {
    size_t c = neighbors[2];
    double dc = 1.4;
    bond = cc.rt.find_bond(cc.atoms[center].id, cc.atoms[c].id);
    if (bond != cc.rt.bonds.end()) {
      double value = std::isfinite(bond->value) ? bond->value : bond->value_nucleus;
      if (std::isfinite(value))
        dc = value;
    }
    double theta_ac = rad(default_angle_for_center(adjacency, center));
    if (const Restraints::Angle* angle = find_angle_restraint(cc, atom_index, a, center, c))
      if (std::isfinite(angle->value))
        theta_ac = rad(angle->value);
    double theta_bc = rad(default_angle_for_center(adjacency, center));
    if (const Restraints::Angle* angle = find_angle_restraint(cc, atom_index, b, center, c))
      if (std::isfinite(angle->value))
        theta_bc = rad(angle->value);
    std::pair<Position, Position> pair = position_from_two_angles_local(
        cc.atoms[center].xyz, cc.atoms[a].xyz, cc.atoms[b].xyz, dc, theta_ac, theta_bc);
    if (std::isfinite(pair.first.x) && std::isfinite(pair.second.x)) {
      double s1 = placement_contact_score(cc, pair.first, c, center);
      double s2 = placement_contact_score(cc, pair.second, c, center);
      cc.atoms[c].xyz = s1 <= s2 ? pair.first : pair.second;
      cc.atoms[c].xyz.z = 0.0;
    }
  }
  return true;
}
...
int generate_chemcomp_xyz_from_restraints(ChemComp& cc) {
  ...
  std::vector<std::vector<size_t>> small_plane_groups = collect_small_plane_groups(cc, atom_index);
  std::vector<std::vector<size_t>> large_plane_groups = collect_large_plane_groups(cc, atom_index);
  ...
  while (count_finite(true) < heavy_count) {
    ...
    bool seeded_large_plane = false;
    for (const auto& group : large_plane_groups) {
      if (std::find(group.begin(), group.end(), seed) != group.end() &&
          seed_large_plane_group_coordinates(cc, adjacency, atom_index, group,
                                             component * 8.0)) {
        seeded_large_plane = true;
        break;
      }
    }
    if (seeded_large_plane) {
      bool progress = true;
      while (progress) {
        progress = false;
        for (size_t i = 0; i != n; ++i)
    ...
    for (const auto& group : small_plane_groups) {
      if (std::find(group.begin(), group.end(), seed) != group.end() &&
          seed_small_plane_group_coordinates(cc, adjacency, atom_index, group,
                                             component * 8.0)) {
        seeded_plane = true;
        break;
      }
